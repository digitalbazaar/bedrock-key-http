...
function addRoutes(app) {
  ...
  app.post(keyPath,
    ...
    (req, res, next) => {
      ...
      // add public key
      brKey.addPublicKey(
        {actor: req.user.identity, publicKey, privateKey}, err => {
  ...
  function _postPublicKey(publicKeyId, req, res, next) {
    ...
    if('revoked' in req.body) {
      ...
      return brKey.revokePublicKey(
        {actor: req.user.identity, publicKeyId}, (err, key) => {
          if(err) {
            return next(err);
          }
    ...
    async.waterfall([
      callback => {
        ...
        brKey.getPublicKey({publicKey}, callback);
      },
      ({publicKey}, callback) => {
        // update public key
        ...
        brKey.updatePublicKey({actor: req.user.identity, publicKey}, callback);
      }
    ], err => {
      if(err) {
        return next(err);
  ...
  if(identityBasePath) {
    ...
    app.get(identityBasePath + '/keys/:publicKey',
      ...
      brRest.linkedDataHandler({
        get: (req, res, callback) => {
          // get old ID from URL
          const publicKey = {id: _createOldPublicKeyId(req)};
          // get public key
          brKey.getPublicKey({publicKey}, (err, result) => {
